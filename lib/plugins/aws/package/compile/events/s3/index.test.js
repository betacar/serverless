...
describe('AwsCompileS3Events', () => {
  ...
  describe('#existingS3Buckets()', () => {
    ...
    beforeEach(() => {
      awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate.Resources.IamRoleLambdaExecution = {
        Properties: {
          Policies: [
            {
              PolicyDocument: {
    ...
    it('should create the necessary resources for the most minimal configuration', () => {
      ...
      expect(Resources.IamRoleLambdaExecution).to.deep.equal({
        Properties: {
          Policies: [
            {
              PolicyDocument: {
                Statement: [
                  {
                    Effect: 'Allow',
                    Resource: 'arn:aws:s3:::existing-s3-bucket',
